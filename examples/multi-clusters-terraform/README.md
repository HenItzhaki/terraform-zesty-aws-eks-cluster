...
This example shows how to onboard one AWS account to Zesty and then deploy the Kompass Helm chart into multiple Amazon EKS clusters by using separate Terraform states.

Use this example when the customer wants account onboarding and each cluster installation to be managed independently.
...
The `account/` stack registers the AWS account with Zesty and stores the generated `kompass_values_yaml` output in Terraform remote state.
...
- AWS credentials with permission to create IAM resources and read the target EKS clusters.
...
| `account/provider.tf` | `region = "us-east-1"` | AWS region for the account-level deployment |
...
| `kompass-eks-*/main.tf` | `zesty/kompass-eks-prod/terraform.tfstate` | Unique backend key for each cluster stack |
...
Important: every cluster stack must use a unique Terraform backend `key`. Do not allow staging, data, and production clusters to share the same state key.
...
- The backend `key` with a value unique to this cluster stack.
- The remote state `bucket`, `key`, and `region` with the exact values used by the `account/` stack.

Use a unique backend key for each cluster:
...
Open the cluster stack's `provider.tf` and update the EKS cluster name:
...
Open the cluster stack's `variables.tf` and update the AWS region if required:
