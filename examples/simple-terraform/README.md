...
This example shows how to onboard one AWS account to Zesty and deploy the Kompass Helm chart into one Amazon EKS cluster from a single Terraform root.
...
1. Onboards the AWS account to Zesty by calling the module in the repository root.
...
├── variables.tf   # EKS cluster name input
...
- AWS credentials with permission to create IAM resources and read the target EKS cluster.
...
|----------|-------------|----------------|
| `provider.tf` | `region = "us-east-1"` | AWS region where the EKS cluster exists |
...
### Step 2: Configure the AWS Region and Zesty Token
...
- `us-east-1` with the AWS region where the target EKS cluster exists.
- `your-zesty-api-token` with the customer Zesty API token.

### Step 3: Identify the EKS Cluster Name
...
- The AWS provider region is correct.
...
Terraform onboards the AWS account to Zesty and installs Kompass into the selected EKS cluster.
...
| `variables.tf` | Defines the EKS cluster name input |
