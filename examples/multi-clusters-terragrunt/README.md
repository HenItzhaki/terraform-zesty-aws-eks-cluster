...
This example shows how to onboard one AWS account to Zesty and then deploy the Kompass Helm chart into multiple Amazon EKS clusters by using Terragrunt.
...
- `account/` onboards the AWS account to Zesty.
...
Terragrunt applies the account stack first. The cluster stacks can then be applied independently or together with `terragrunt run-all apply`.
...
The backend `key` is generated from the Terragrunt directory path, so `account/`, `kompass-eks-prod/`, and `kompass-eks-staging/` receive separate state files automatically.
...
The account stack uses the published module source:
...
2. Rename the copied directory to match the new cluster purpose, for example `kompass-eks-data/`.
...
| `live/prod/aws/us-east-1/my-account/zesty/account/terragrunt.hcl` | Calls the published Zesty module and generates the Zesty provider |
